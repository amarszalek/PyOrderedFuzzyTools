...
class OFCandle(OFNumber):
    ...
    def to_ofn(self):
        return OFNumber(self.branch_f, self.branch_g)
...
def compute_linear(price, volume, spread, params):
    ...
    if price[0] <= price[-1]:
        ...
        ofc = ofnumber.init_trapezoid_x0x1(fx0, fx1, gx0, gx1, dim=params['dim'])
    else:
...
def compute_gauss(price, volume, spread, params):
    ...
    if price[0] <= price[-1]:
        ...
        ofc = ofnumber.init_gaussian(mf, sf, mg, sg, dim=params['dim'])
    else:
        mf = s2
